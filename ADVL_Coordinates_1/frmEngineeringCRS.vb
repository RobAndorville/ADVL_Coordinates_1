Public Class frmEngineeringCRS
    'The Engineering Coordinate Reference System form is used to view, create or edit Engineering CRS parameters.
...
#Region " Form Display Methods - Code used to display this form." '----------------------------------------------------------------------------------------------------------------------------
    ...
    Private Sub frmEngineeringCRS_Load(sender As Object, e As EventArgs) Handles Me.Load
        RestoreFormSettings()   'Restore the form settings

        Main.AreaOfUse.AddUser()
        Main.Datum.AddUser()
        Main.CoordinateSystem.AddUser()
        Main.EngineeringCRS.AddUser()

        UpdateList()
        txtNRecords.Text = Main.EngineeringCRS.NRecords
        txtEngineeringCRSListFileName.Text = Main.EngineeringCRS.ListFileName
        CurrentRecordNo = 1
        DisplayListData(1)
    End Sub
    ...
    Private Sub btnExit_Click(sender As Object, e As EventArgs) Handles btnExit.Click
        ...
        Main.EngineeringCRS.RemoveUser()

        Me.Close() 'Close the form
    End Sub

    Private Sub frmEngineeringCRS_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
...
#Region " Form Methods - The main actions performed by this form." '---------------------------------------------------------------------------------------------------------------------------
    ...
    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        'Save the Engineering CRS list.

        Dim EngineeringCRSListFileName As String = Trim(txtEngineeringCRSListFileName.Text)

        If EngineeringCRSListFileName = "" Then
            Main.Message.SetWarningStyle()
            Main.Message.Add("Please enter a file name for the Engineering CRS list!" & vbCrLf)
            Exit Sub
        End If

        If EngineeringCRSListFileName.EndsWith(".EngineeringCRSList") Then
            'EngineeringCRSListFileName has correct file extension.
            Main.EngineeringCRS.ListFileName = EngineeringCRSListFileName
        Else
            'Add file extension to the file name.
            EngineeringCRSListFileName &= ".EngineeringCRSList"
            Main.EngineeringCRS.ListFileName = EngineeringCRSListFileName
            txtEngineeringCRSListFileName.Text = EngineeringCRSListFileName
        End If
        Main.Project.SaveXmlData(EngineeringCRSListFileName, Main.EngineeringCRS.ToXDoc())
    End Sub

    Private Sub btnGetEpsgList_Click(sender As Object, e As EventArgs) Handles btnGetEpsgList.Click
        'Get the Engineering CRS list from the EPSG database.
    ...
    Private Sub UpdateList()
        ...
        For Index = 0 To Main.EngineeringCRS.NRecords - 1
            ListBox1.Items.Add(Main.EngineeringCRS.List(Index).Name)
        Next
    End Sub

    Private Sub DisplayListData(ByVal RecordNo As Integer)
        'Display a record in the Engineering CRS list.
        ...
        If RecordNo > Main.EngineeringCRS.NRecords Then
            Main.Message.SetWarningStyle()
            Main.Message.Add("Cannot display CRS data. Selected record number is too large." & vbCrLf)
            Main.Message.Add("RecordNo = " & RecordNo & "   Main.EngineeringCRS.NRecords = " & Main.EngineeringCRS.NRecords & vbCrLf)
            Exit Sub
        End If

        txtCRSName.Text = Main.EngineeringCRS.List(RecordNo - 1).Name
        txtCRSName2.Text = Main.EngineeringCRS.List(RecordNo - 1).Name
        txtCrsAuthor.Text = Main.EngineeringCRS.List(RecordNo - 1).Author
        txtCrsCode.Text = Main.EngineeringCRS.List(RecordNo - 1).Code
        txtCrsDeprecated.Text = Main.EngineeringCRS.List(RecordNo - 1).Deprecated

        'Update the list of alias names:
        cmbCRSAliasNames.Items.Clear()
        cmbCRSAliasNames.Text = ""
        For Each item As String In Main.EngineeringCRS.List(RecordNo - 1).AliasName
        ...
        txtCrsScope.Text = Main.EngineeringCRS.List(RecordNo - 1).Scope
        txtCrsComments.Text = Main.EngineeringCRS.List(RecordNo - 1).Comments

        'Display Area Of Use data:
        txtAOUName.Text = Main.EngineeringCRS.List(RecordNo - 1).Area.Name
        DisplayAOUData(Main.EngineeringCRS.List(RecordNo - 1).Area.Author, Main.EngineeringCRS.List(RecordNo - 1).Area.Code)
    ...
    Private Sub FindRecord(ByVal SearchString As String)
        ...
        FoundIndex = Main.EngineeringCRS.List.FindIndex(Function(x) x.Name.Contains(SearchString))
    ...
    Private Sub FindPrevRecord(ByVal SearchString As String)
        ...
        If CurrentRecordNo > 1 Then
            ...
            FoundIndex = Main.EngineeringCRS.List.FindLastIndex(Start, Function(x) x.Name.Contains(SearchString))
            If FoundIndex = -1 Then
                Main.Message.Add("String not found." & vbCrLf)
            Else
                CurrentRecordNo = FoundIndex + 1
    ...
    Private Sub FindNextRecord(ByVal SearchString As String)
        Dim FoundIndex As Integer
        FoundIndex = Main.EngineeringCRS.List.FindIndex(CurrentRecordNo, Function(x) x.Name.Contains(SearchString))
        If FoundIndex = -1 Then
            Main.Message.Add("String not found." & vbCrLf)
        Else
            CurrentRecordNo = FoundIndex + 1
    ...
    Private Sub txtRecordNo_TextChanged(sender As Object, e As EventArgs) Handles txtRecordNo.TextChanged
        ...
        If NewRecordNo > Main.EngineeringCRS.NRecords Then
            Exit Sub
        End If
    ...
    Private Sub btnNext_Click(sender As Object, e As EventArgs) Handles btnNext.Click
        ...
        If CurrentRecordNo = Main.EngineeringCRS.NRecords Then
    ...
    Private Sub btnLast_Click(sender As Object, e As EventArgs) Handles btnLast.Click
        ...
        CurrentRecordNo = Main.EngineeringCRS.NRecords
        DisplayListData(CurrentRecordNo)
    End Sub
    ...
    Private Sub btnFind_Click(sender As Object, e As EventArgs) Handles btnFind.Click
        ...
        Select Case Main.Project.DataLocn.Type
            Case ADVL_Utilities_Library_1.FileLocation.Types.Directory
                ...
                OpenFileDialog1.Filter = "Engineering CRS List | *.EngineeringCRSList"
                If OpenFileDialog1.ShowDialog() = Windows.Forms.DialogResult.OK Then
                    Dim DataFileName As String = System.IO.Path.GetFileName(OpenFileDialog1.FileName)
                    txtEngineeringCRSListFileName.Text = DataFileName
                    Main.EngineeringCRS.ListFileName = DataFileName
                    Dim XmlDoc As System.Xml.Linq.XDocument
                    Main.Project.DataLocn.ReadXmlData(DataFileName, XmlDoc)
                    Main.EngineeringCRS.LoadXml(XmlDoc)
                    UpdateList()
                    txtNRecords.Text = Main.EngineeringCRS.NRecords
                    txtRecordNo.Text = 1
                    DisplayListData(1)
            ...
            Case ADVL_Utilities_Library_1.FileLocation.Types.Archive
                ...
                'Dim Zip As New ADVL_System_Utilities.ZipComp
                ...
                Zip.SelectFileForm.FileExtension = ".EngineeringCRSList"
    ...
    Private Sub Zip_FileSelected(FileName As String) Handles Zip.FileSelected
        txtEngineeringCRSListFileName.Text = FileName
